...
async function defaultArgs(provider: AgentProvider, port: number): Promise<string[]> {
  ...
  // Codex one-off config overrides merge this live server with the user's own
  // config.toml without persisting anything there. Do not override the user's
  // model, sandbox, or approval policy.
  return [
...
export function registerAgentIpc(getWin: () => BrowserWindow | null) {
  ...
  // Compatibility surface for scripts using the old Claude-only preload API.
